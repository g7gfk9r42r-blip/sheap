...
- Normalisiert Schema
- Generiert Bilder via Replicate API oder SD
- Schreibt nach assets/recipes/<market>/<market>_recipes.json
...
from schema_adapter import normalize_recipe

try:
    from replicate_image import ReplicateImageClient
except ImportError:
    ReplicateImageClient = None

try:
    from sd_image import StableDiffusionImageClient
except ImportError:
    StableDiffusionImageClient = None


class WeeklyRefreshPipeline:
    """Haupt-Pipeline für wöchentliche Rezept-Aktualisierung (Offline-First)"""
    ...
    def __init__(
        ...
        if image_backend == "replicate":
            if ReplicateImageClient is None:
                print("⚠️  WARNING: ReplicateImageClient nicht verfügbar")
                if strict:
                    sys.exit(1)
            else:
                try:
                    self.image_client = ReplicateImageClient(model=replicate_model)
                except ValueError as e:
                    print(f"⚠️  WARNING: {e}")
                    if strict:
                        sys.exit(1)
                except Exception as e:
                    print(f"⚠️  WARNING: Replicate Client Fehler: {e}")
                    if strict:
                        sys.exit(1)
        elif image_backend == "sd":
            if StableDiffusionImageClient is None:
                print("⚠️  WARNING: StableDiffusionImageClient nicht verfügbar")
                if strict:
                    sys.exit(1)
            else:
                try:
                    self.image_client = StableDiffusionImageClient(sd_url)
                    is_connected, error_msg = self.image_client.check_connection()
                    if not is_connected:
                        print(f"⚠️  WARNING: SD nicht erreichbar: {error_msg}")
                        if strict:
                            sys.exit(1)
                except Exception as e:
                    print(f"⚠️  WARNING: SD Client Fehler: {e}")
                    if strict:
                        sys.exit(1)
        elif image_backend == "none":
            print("ℹ️  Image-Backend: none (keine Bilder werden generiert)")
        else:
            print(f"⚠️  WARNING: Unbekanntes Image-Backend: {image_backend}")
    ...
    def validate_recipe_schema(self, recipe: Dict, index: Optional[int] = None) -> Tuple[bool, Optional[str], Optional[str]]:
        """
        Validiert ein normalisiertes Rezept.
        Returns: (is_valid, error_message, json_path)
        """
        ...
        # Title muss existieren
        title = recipe.get('title')
        if not title:
            return False, "Missing required field: title", f"{prefix}.title"
        
        if not str(title).strip():
            return False, "Title is empty", f"{prefix}.title"
        
        # Categories/Tags (optional, aber wenn vorhanden muss es Array sein)
        categories = recipe.get('categories', [])
        if categories is not None and not isinstance(categories, list):
            return False, "categories must be an array", f"{prefix}.categories"
        
        # Servings (muss vorhanden sein, default 1)
        servings = recipe.get('servings', 1)
        try:
            servings_int = int(servings)
            if servings_int < 1:
                return False, "servings must be >= 1", f"{prefix}.servings"
        except:
            return False, "servings must be an integer", f"{prefix}.servings"
        
        # Ingredients - muss Array sein (kann leer sein)
        ingredients = recipe.get('ingredients', [])
        if not isinstance(ingredients, list):
            return False, "ingredients must be an array", f"{prefix}.ingredients"
        
        # Prüfe Angebotszutaten (from_offer=true müssen alle Felder haben)
        for idx, ing in enumerate(ingredients):
            if isinstance(ing, dict) and ing.get('from_offer') is True:
                required_fields = ['offer_id', 'name', 'price_eur']
                for field in required_fields:
                    if field not in ing or ing[field] is None:
                        return False, f"Missing required field in ingredient[{idx}]: {field}", f"{prefix}.ingredients[{idx}].{field}"
        
        # Extra Ingredients - Array (optional)
        extra_ingredients = recipe.get('extra_ingredients', [])
        if extra_ingredients is not None and not isinstance(extra_ingredients, list):
            return False, "extra_ingredients must be an array", f"{prefix}.extra_ingredients"
        
        # Instructions/Steps muss existieren
        steps = recipe.get('steps')
        if not steps:
            return False, "Missing required field: steps", f"{prefix}.steps"
        
        if isinstance(steps, list) and len(steps) == 0:
            return False, "Steps list is empty", f"{prefix}.steps"
        
        if isinstance(steps, str) and not steps.strip():
            return False, "Steps string is empty", f"{prefix}.steps"
    ...
    def process_market(self, market_key: str, input_file: Path) -> bool:
        ...
        # Normalisiere und validiere Rezepte
        valid_recipes = []
        seen_ids = set()
        ...
        for idx, recipe in enumerate(recipes):
            ...
            # Validierung
            is_valid, error_msg, json_path = self.validate_recipe_schema(normalized, index=idx)
            if not is_valid:
                recipe_id = normalized.get('id', 'unknown')
        ...
        # WICHTIG: Output muss exakt gleich viele Rezepte haben wie Input (in non-strict: nur valide)
        if self.strict and valid_count != recipes_count:
        ...
        if not self.dry_run:
            ...
            for idx, recipe in enumerate(valid_recipes):
                ...
                # Erwarteter Bild-Pfad (PNG Format)
                # NEUE STRUKTUR: assets/images/recipes/<market>_R001.png (flach, keine Unterordner)
                image_filename = f"{recipe_id}.png"
                image_filename_with_market = f"{market_key}_{image_filename}"
                image_path = self.images_dir / image_filename_with_market
                image_path_str = f"assets/images/recipes/{image_filename_with_market}"
                
                # Proaktives Throttling: Warte ZWISCHEN Bildern (außer beim ersten Bild)
                if idx > 0 and self.throttle_ms > 0:
                ...
                if self.image_client:
                    success, error_msg = self.image_client.generate_image(
                        ...
                        throttle_ms=0,  # Throttling wird hier in der Loop gemacht
                    )
        ...
        # WICHTIG: Output-Anzahl
        output_count = len(updated_recipes)
        if self.strict and output_count != recipes_count:
        ...
        # WICHTIG: assets/recipes/<market>/<market>_recipes.json
        output_market_dir = self.output_dir / market_key
        output_market_dir.mkdir(parents=True, exist_ok=True)
        output_file = output_market_dir / f"{market_key}_recipes.json"
        
        if self.dry_run:
            print(f"   [DRY RUN] Würde schreiben: {output_file.relative_to(self.output_dir.parent)} ({output_count} Rezepte)")
            ...
            if success:
                self.stats['files_written'] += 1
                print(f"   ✅ Gespeichert: {output_file.relative_to(self.output_dir.parent)} ({output_count} Rezepte)")
            else:
    ...
    def run(self) -> int:
        ...
        print("🔄 Weekly Recipe Refresh Pipeline - Offline-First")
        print("=" * 60)
    ...
    def print_report(self):
        ...
        for market in sorted(set(list(self.stats['recipes_loaded'].keys()) + list(self.stats['recipes_output'].keys()))):
            ...
            # Prüfe Konsistenz
            if loaded != output and self.strict:
                print(f"      ⚠️  WARNING: Input ({loaded}) != Output ({output})")
        
        if not self.dry_run and self.image_client:
            print(f"\n🖼️  Bilder:")
...
def main():
    parser = argparse.ArgumentParser(
        description='Weekly Recipe Refresh Pipeline - Offline-First',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Beispiele:
  # Dry Run (Validation only)
  python3 tools/weekly_refresh.py --input assets/prospekte --out assets/recipes --images assets/images/recipes --image-backend none --dry-run --strict
  
  # Full Run (Replicate)
  export REPLICATE_API_TOKEN="r8_..."
  python3 tools/weekly_refresh.py --input assets/prospekte --out assets/recipes --images assets/images/recipes --image-backend replicate --strict
  
  # Nur einen Market testen
  python3 tools/weekly_refresh.py --input assets/prospekte --out assets/recipes --images assets/images/recipes --image-backend none --dry-run --strict --only aldi_nord
        """
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default='none',
        choices=['replicate', 'replica', 'sd', 'none'],
        help='Image Backend: replicate (Standard), replica (Alias), sd (Stable Diffusion), none'
    )
    ...
    parser.add_argument(
        ...
        help='Replicate Model (Standard: black-forest-labs/flux-schnell)'
    )
    ...
    parser.add_argument(
        ...
        help='Stable Diffusion API URL (Standard: http://127.0.0.1:7860)'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=200,
        help='Throttle-Delay in Millisekunden ZWISCHEN jedem Bild-Request (Standard: 200ms = 5 req/s, sicher unter 600/min Limit). Erhöhen bei Rate Limits (z.B. 1000-2000). Setze auf 0 für kein Throttling.'
    )
    ...
    # Normalisiere image-backend (replica -> replicate)
    image_backend = args.image_backend
    if image_backend == 'replica':
        image_backend = 'replicate'
        print("ℹ️  'replica' wird zu 'replicate' normalisiert")
